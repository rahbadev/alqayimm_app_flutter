...
class TestDownloaderScreen extends StatefulWidget {
  const TestDownloaderScreen({super.key});

  @override
  State<TestDownloaderScreen> createState() => _TestDownloaderScreenState();
}

class _TestDownloaderScreenState extends State<TestDownloaderScreen> {
  ...
  void initState() {
    ...
    FileDownloader()
        ...
        .then((result) => logger.d('Configuration result = $result'));
  ...
  void myNotificationTapCallback(Task task, NotificationType notificationType) {
    logger.d('Tapped notification $notificationType for taskId ${task.taskId}');
  }
              ...
              children: [
                ...
                Center(
                  child: ElevatedButton(
                    ...
                    child: const Text('Load in background'),
                  ),
                ),
                Center(
                  child: Text(
    ...
    if (!loadAndOpenInProgress) {
      ...
      if (Platform.isIOS) {
        ...
        if (auth == PermissionStatus.granted) {
          ...
          logger.d('iOS Photo Library permission not granted');
        }
      }
      if (Platform.isAndroid) {
        ...
        if (auth == PermissionStatus.granted) {
          ...
          logger.d('androidSharedStorage permission not granted');
        }
      }
      setState(() {
  ...
  Future<void> processPickDirectory() async {
    ...
    logger.i('Download to URI completed with taskStatus ${result.status}');
    logger.i('Downloaded file is at ${resultTask.fileUri}');
    logger.i('Downloaded file name is ${resultTask.filename}');
  }
  ...
  Future<void> getPermission(PermissionType permissionType) async {
    ...
    if (status != PermissionStatus.granted) {
      if (await FileDownloader().permissions.shouldShowRationale(
        ...
        logger.d('Showing some rationale');
      }
      status = await FileDownloader().permissions.request(permissionType);
      logger.d('Permission for $permissionType was $status');
    }
  }
}
